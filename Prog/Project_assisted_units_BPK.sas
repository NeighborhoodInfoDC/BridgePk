/**************************************************************************
 ...
 Modifications:
			   - 3/20/17, JD: Modified to produce summary tables for 11th
				 Street Bridge Park
**************************************************************************/
...
proc sort data=Assisted_units 
    out=BridgePk.Project_assisted_units 
          (label="Preservation Catalog, Assisted unit counts by project and subsidy portfolio");
  by ProgCat NLIHC_ID;

%File_info( data=BridgePk.Project_assisted_units, printobs=0, freqvars=ProgCat Ward2012 Geo2010 )

proc print data=BridgePk.Project_assisted_units n='Projects = ';
  by ProgCat;
  id NLIHC_ID;
...
ods rtf file="&_dcdata_r_path\BridgePk\Prog\Project_assisted_units.rtf" style=Styles.Rtf_arial_9pt;
...
proc tabulate data=BridgePk.Project_assisted_units format=comma10. noseps missing;
  where ProgCat ~= .;
...
proc tabulate data=BridgePk.Project_assisted_units format=comma10. noseps missing;
  ...
  table 
    ...
    all='\b Total' ProgCat=' ',
    /** Columns **/
    sum='Assisted Units by Tract' * geo2010=' ' * ( mid_asst_units='Est.' err_asst_units='+/-' )
    ;
  format ProgCat ProgCat.;
  title3 "Project and assisted unit unique counts";
run;
